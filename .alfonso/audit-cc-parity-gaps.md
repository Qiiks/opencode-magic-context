...
> **Joint design pinned (2026-08-14, with Thalamus):** (1) projection APPENDS after all byte-stable system blocks — Anthropic matches tools→system→messages, so content ahead of the anchor pays its bytes back as creation on every fold (measured: system is ~12,614 tokens of the ~56,654-token stable floor; the build-id truncation event is the observed failure shape); (2) the gateway anchor must end the STABLE PREFIX, not the array — consolidate_system_anchor's blocks.last_mut() would silently anchor a variable appended block; this change lands FIRST (independently safe); (3) projected block + m0 rematerialize together on publish — accounting neutral by construction. Ordering semantics settled: m0 absorption already hoists covered reminders out of position, so projection preserves existing interval semantics exactly. Sequencing ladder: parity R3 → real OAuth session over the deployed anchor build (anti-spoofing attribution) → coordinated flip.
