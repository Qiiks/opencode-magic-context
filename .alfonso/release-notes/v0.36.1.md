...
**Repeated cache invalidation under sustained context pressure.** Once a session touched the emergency band, it stayed in that mode for up to 30 minutes — and during that time each newly superseded tool result was dropped on its own pass, invalidating the prompt cache every time. Those drops now wait and land together with work that was already rewriting the request. Measured on a live session: 55 avoidable invalidations in 30 minutes.
