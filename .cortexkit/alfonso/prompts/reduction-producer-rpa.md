# Implement: reduction-producer slice RP-A (design doc embedded below)

Branch: subc-migration (fork from HEAD). Implement slice RP-A of the v4 design EXACTLY as written in the embedded doc: P0 config expansion, P1 scheduler wiring with the producer gate, P2 selection wiring + _decider DELETION, P3 state substrate, P4 durable agent-drop queue with the commit_with_consumed_drops variant. Slice RP-B (P5, the >=95% blocking arm) is NOT in scope — do not touch the historian spawn path.

Every R-marked clause (R1-Fn/R2-Fn/R3-Fn) is load-bearing — do not simplify. Key pins to honor:
- Producer gate: selection runs on scheduler Execute/EmergencyForce OR hard_fold_requested advisory (the TS BUST-clause). Never on defer.
- Candidate pool: tail_sel_items(live, coverage_ordinal), never the full projection.
- current_total_input_tokens = request ModuleUsage fill number. Also FIX the stale doc comment on SelectionContext.current_total_input_tokens in selection.rs (it still says module-derived-over-composed; round 2 flagged it).
- Two-pass selector disabled while reconcile_pending; re-cut arm clamps last_execute_ordinal (None coverage = 0) in its commit.
- tail_state open-arc detection excludes provider_executed blocks.
- cache_ttl default "5m" via SessionMeta; SessionBinding.model_key: Option<String> populated None for now.
- last_committed_pass_at_ms set only AFTER the changed-comparison decides the commit.
- pending_agent_drops: new mc-store table + append command handled in lib.rs (demuxed by method on the session route, same BindIdentity checks as transform) + drained into the frozen set on busting passes with consumed rows deleted via commit_with_consumed_drops INSIDE the same fenced tx; idempotent against frozen set; unresolvable ids consumed and dropped.
- DeciderInputs/_decider field: DELETE from the request struct. Migrate existing tests: behavior tests drive selection through crafted live arrays; monotonicity/adversarial tests that need hand-crafted reductions use a #[cfg(test)] injection hook on the handler.
- Invariant 5 test: a pure-defer pass commits nothing (row_version stable) with the new fields present.

Tests required (beyond migrating existing): producer-gate matrix (execute/force/defer/hard-advisory), coverage-filtered pool (covered blocks never selected), agent-drop queue end-to-end (append command -> drain on next busting pass -> rows deleted same tx -> crash-before-commit keeps rows -> resend idempotent), reconcile disables two-pass + clamp restores watermark, provider_executed open arc does not defer, TTL-hard requires in-process observation (durable anchor alone never fires), execute-with-zero-delta lands byte-identical (defer-shaped, no commit).

Gates: cargo test -p mc-module -p mc-core -p mc-store --features mc-store/test-support; cargo test -p mc-module --test real_daemon; cargo clippy --workspace --all-targets -- -D warnings; cargo fmt --check; check_comments. Commits explain WHY without referencing rounds/briefs.

----- DESIGN DOC (v4, verbatim) -----
