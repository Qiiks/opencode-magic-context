# Magic Context v0.38.1

Patch release: per-harness model configuration, calmer context-housekeeping nudges, and a batch of reliability fixes across the dreamer, historian, and command surfaces.
...
- **Tool-set change telemetry** (#349): fold decisions now record a name-only tool-set hash plus before/after operands, so provider prefix busts caused by tool-list changes can be diagnosed from the decision table — without reintroducing tool-set-triggered folding.
- **Historian child cleanup** (#336) rides the age-gated sweep (and the sweep now runs with the dreamer disabled); the orphan sweep covers all session-scoped tables (#346).
- **Provider translation** (#344): `opencode` ↔ `opencode-zen` model references translate correctly on OMP.
...
- Windows/CI: TUI probe workflow fixes (#330, #333), lockfile-pinned CI installs, and test-suite dependency preflights.

## Notes

Database schema moves to v80 (fold-telemetry columns; additive). No configuration action is required — the per-harness migration is automatic and keeps a backup.
