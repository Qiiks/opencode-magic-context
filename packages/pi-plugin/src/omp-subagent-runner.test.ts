...
describe("OmpSubagentRunner", () => {
	...
	test("hostContext.modelRegistry rides the session into the structured request", async () => {
		...
		const fakeRegistry = { authStorage: {}, getAvailable: () => [] };
		const runner = new OmpSubagentRunner({ surface: surface as never });
		...
		await runner.run({ ...BASE_OPTIONS, model: "bai/glm-5.3-flash:high", thinkingLevel: "high" });
		const request = capture.requests[0]!;
		const session = request.session as Record<string, unknown>;
		// The parent's registry must be forwarded so OMP's runSubprocess sets
		// registryFromParent and reuses it instead of building a fresh bundled-
