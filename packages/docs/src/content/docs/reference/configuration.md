...
Magic Context reads `magic-context.jsonc` (or `.json`) from one shared CortexKit location, the same for both harnesses. Project config overrides user config, key by key. Cache-TTL routing uses the case-sensitive model-key walk: exact provider/model, bare model, progressively shorter hyphen-suffixes, provider/*, then the configured default. Prompt-surface routing uses the provider/model suffix and wildcard order (bare model keys are not valid there). Prompt-surface routing is shared by both harnesses; project config may select `default` and `models`, while `guidance_override_path` and `tool_descriptions` are stripped at the project trust boundary.
...
| `todowrite` | object | — | Pi-only todowrite tool and overlay controls. If absent, it defaults to enabled. Pi registers tools and widgets at extension boot, so changing this after /cd requires /reload or restart. |
...
Select the full or light built-in prompt preset. Model routes use the same provider/model suffix and provider/* order as `cache_ttl`, with literal case-sensitive keys; bare model keys are not valid for `prompt_surface`. Guidance and tool-description overrides are user-level only.

| Key | Type | Default | Description |
|---|---|---|---|
| `prompt_surface` | object | — | Prompt-surface presets: default is full; models use the literal provider/model or provider/* routing grammar. Guidance and tool-description overrides are user-level only. On OpenCode and Pi, per-model routing applies to the guidance block only — tool descriptions are registered once per process and follow `prompt_surface.default` (v1 plugin-surface limitation; per-model tool descriptions are planned for the OpenCode v2 plugin API once the SDK stabilizes). |
...
| Key | Type | Default | Description |
|---|---|---|---|
| `cache_ttl` | string \\| map<string, string> | `"5m"` | Cache TTL: string (e.g. "5m", "1h", "30s") or per-model object ({ default: "5m", "model-id": "10m" }). Per-model lookup is exact provider/model → bare model → progressively shorter hyphen-suffixes → provider/* → `default`; keys are case-sensitive. Set to `"never"` for a lane kept warm by an external keepwarm proxy — disables the idle-TTL heuristic so MC never initiates a rebuild based on elapsed time. |
...
Magic Context resolves the model's usable input budget from the harness: OpenCode uses its SDK-resolved provider metadata, while Pi uses the active model's context window. A smaller limit observed during the session can narrow the budget; when no trusted model limit is available, runtime pressure math uses a conservative fallback. `output_reserve` subtracts shared-window output capacity before the execute threshold and history budget are applied; an explicit provider input limit wins without double-reserving output. Set `output_reserve: 0` to disable that reservation.
...
| `shadow_embedding` | object | `{"enabled":false}` | Developer-only shadow embedding comparison setting. Leave unset unless you are running the local Synapse development setup. |
| `shadow_embedding.enabled` | boolean | `false` | Developer-only shadow embedding switch. |
| `transform_mode` | `"ts"` \\| `"rust"` | `"ts"` | Developer-only runtime selection. The Rust mode requires a local `subc` daemon; leave this unset for the supported TypeScript runtime. |
