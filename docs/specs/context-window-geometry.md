...
|---|---|---|---|
| GPT-5.6 family, ChatGPT OAuth / Codex backend | **372k shared admitted**; advertised (server-delivered `ModelInfo.context_window`) moved 372k → 272k (source-verified: codex-rs, openai/codex#31860) | backend accepts no `max_output_tokens` field at all (source-verified) | advertised vs enforcement differ (stated, operator-confirmed live: 340k+ input accepted). Volatility watch: enforcement may tighten toward 272k; a provider-proven overflow between 272k and 372k is the canary |
