...
- `src/features/magic-context/migrations.ts`: Versioned schema migrations v1–v64 (`LATEST_SUPPORTED_VERSION` in `storage-db.ts` must track the highest; `schema-version-fence.test.ts` asserts they stay in lockstep).
